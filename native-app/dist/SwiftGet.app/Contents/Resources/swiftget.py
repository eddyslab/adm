...
import logging
import subprocess
import rumps  # macOS menu bar — installed via pip
...
class MenuBarApp(rumps.App):
    def __init__(self, tk_app: SwiftGetApp, engine: DownloadEngine):
        super().__init__(APP_NAME, icon=None, quit_button="종료")
        self.tk_app = tk_app
        self.engine = engine
        self.menu = ["SwiftGet 열기", "다운로드 폴더", None]

    @rumps.clicked("SwiftGet 열기")
    def show_window(self, _):
        self.tk_app.deiconify()
        self.tk_app.lift()
        self.tk_app.focus_force()

    @rumps.clicked("다운로드 폴더")
    def open_folder(self, _):
        subprocess.Popen(["open", SAVE_DIR])

    def update_title(self, running: int, speed: float):
        if running > 0:
            self.title = f"↓ {human_speed(speed)}"
        else:
            self.title = APP_NAME
    ...
    def handle(conn):
        try:
            ...
            if msg.get("action") == "download":
                ...
                tk_app.after(0, lambda: (tk_app.deiconify(), tk_app.lift()))
...
def main():
    engine  = DownloadEngine(on_update=lambda: None)  # placeholder
    tk_app  = SwiftGetApp(engine)

    start_socket_server(engine, tk_app)

    # Hide from dock on launch (menu bar only)
    tk_app.withdraw()  # start hidden; show when first download arrives

    # Menu bar (runs in separate thread since rumps has its own run loop)
    mb_app = MenuBarApp(tk_app, engine)

    def run_menubar():
        mb_app.run()

    threading.Thread(target=run_menubar, daemon=True).start()

    # Speed update loop for menu bar title
    def update_menubar():
        while True:
            time.sleep(1)
            ...
            try:
                mb_app.update_title(running, speed)
            except: pass
